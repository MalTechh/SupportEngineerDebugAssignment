...
public class TaskApiTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task ListTasks_ShouldRespectLimit()
    {
        var client = _factory.CreateClient();

        var tasks = await client.GetFromJsonAsync<List<TaskDto>>("/api/tasks?userId=user-001&limit=10");

        tasks.Should().NotBeNull();
        tasks!.Should().HaveCountLessThanOrEqualTo(10);
        tasks.Should().OnlyContain(t => t.UserId == "user-001");
    }

    [Fact]
    public async Task CreateTask_ShouldReturn400_WhenRequiredFieldsMissing()
    {
        var client = _factory.CreateClient();

        var req = new { userId = "", title = "" };
